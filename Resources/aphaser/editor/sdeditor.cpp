...
bool SDEditor::open (void *ptr)
{
	...
	stagesOption = new COptionMenu(size, this, kStages);
	stagesOption->addEntry("2");
	stagesOption->addEntry("4");
	stagesOption->addEntry("6");
	stagesOption->addEntry("8");
	stagesOption->addEntry("10");
	stagesOption->setCurrent((int)(effect->getParameter(kStages)*5.0));
	lFrame->addView (stagesOption);

	// Mix mode
	size.offset( 0, kSelectInc );
	mixOption = new COptionMenu(size, this, kMixMode);
	mixOption->addEntry("normal");
	mixOption->addEntry("-180");
	mixOption->addEntry("wet left");
	mixOption->addEntry("wet right");
	mixOption->addEntry("stereo");
	mixOption->setCurrent((int)(effect->getParameter(kMixMode)*5.0));
...
void SDEditor::setParameter (VstInt32 index, float value)
{
	...
	case kStages:
		if (stagesOption)
			stagesOption->setValue (effect->getParameter (index));
		break;
...
void SDEditor::valueChanged (CDrawContext* context, CControl* control)
{
	...
	case kStages:
		effect->setParameterAutomated (tag, control->getValue () / 5.0f);
		control->setDirty ();
		break;
